...
export async function annotateImage(url: string, boxes: Box[], opts: Opts = {}): Promise<string> {
  ...
  boxes.forEach((b) => {
    const x = Math.round(b.x * canvas.width);
    const y = Math.round(b.y * canvas.height);
    const w = Math.round(b.w * canvas.width);
    const h = Math.round(b.h * canvas.height);
    ...
    const bh = Math.ceil(parseInt(font, 10)) + paddingY * 2;
