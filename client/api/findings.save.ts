    ...
    for (const f of findings) {
      ...
      if (fid) {
        ...
        await client.query(
          `delete from finding_media using media
           where finding_media.finding_id=$1 and media.id=finding_media.media_id and media.report_id=$2`,
          [fid, reportId]
        )
      } else {
      ...
      if (f.mediaRefs.length) {
        ...
        for (const row of mids.rows) {
          await client.query(
            `insert into finding_media(finding_id,media_id) values($1,$2) on conflict do nothing`,
            [fid, row.id]
          )
        }
      }
